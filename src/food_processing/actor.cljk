(ns food-processing.actor
  "ProcessingActor — the ISCO-08 8160 independent food-and-related-
  products machine operator actor as a `langgraph.graph/state-graph`
  (per ADR-2607011000 / CLAUDE.md Actors section). One graph run = one
  processing operation request (intake → advise → govern → decide →
  ...
  The unconditional invariant: the ProcessingAdvisor can never directly
  commit a record or dispatch a robot action the ProcessingGovernor
  refuses — every commit-record! call is gated behind `:decide`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [food-processing.advisor :as advisor]
            [food-processing.governor :as governor]
            [food-processing.store :as store]))

(defn build-graph
  "Build a compiled ProcessingActor graph. `store` implements
  `food-processing.store/Store`. `advisor` implements
  `food-processing.advisor/Advisor` (defaults to `mock-advisor`).
  ...
  (-> (g/state-graph
      ...
      (g/add-node :commit
                   (fn [{:keys [request proposal]}]
                     (let [record {:batch-id (:batch-id request)
                                    :op (:op proposal)
                                    :payload proposal}]
